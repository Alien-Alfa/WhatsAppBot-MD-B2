...
module.exports = {
    async handler(chatUpdate) {
       ...
       // if (chatUpdate.messages.length > 1) console.log(chatUpdate.messages)
        ...
        const alienx = Math.floor(Difference / (1000 * 60 * 60 * 24));
        const alienx1 = Math.floor(Difference % (1000 * 60 * 60 * 24) / (1000 * 60 * 60))
        const alienx2 = Math.floor(Difference % (1000 * 60 * 60) / (1000 * 60))
        const alienx3 = Math.floor(Difference % (1000 * 60) / 1000)
        const alienx4 = new Date('March 9, 2022 23:59:59')
        const alienx5 = new Date().getTime()
        const alienx6 = alienx4 - alienx5
        const alienx7 = Math.floor(alienx6 / (1000 * 60 * 60 * 24));
        const alienx8 = Math.floor(alienx6 % (1000 * 60 * 60 * 24) / (1000 * 60 * 60))
        const alienx9 = Math.floor(alienx6 % (1000 * 60 * 60) / (1000 * 60))
        const alienxx = Math.floor(alienx6 % (1000 * 60) / 1000)
        global.ulangTahun = `Not Enough ${alienx7} Day ${alienx8} O'clock ${alienx9} Minute ${alienxx} Seconds Again`
        global.tahunBaru = `Not Enough ${alienx} Day ${alienx1} O'clock ${alienx2} Minute ${alienx3} Seconds Again`
        ...
        global.dateIndia = Intl.DateTimeFormat("en-US", "Asia/Kolkata");
        ...
        global.sock = msgsz
        global.namabot = msgsz.user.name
        ...
        global.docux = img
        global.nd = pickRandom(['𝘼𝙇𝙄𝙀𝙉 𝘼𝙇𝙁𝘼', 'ᴀʟɪᴇɴ ᴀʟꜰᴀ', 'ＡＬＩＥＮ ＡＬＦＡ', '𝙰𝙻𝙸𝙴𝙽 𝙰𝙻𝙵𝙰', '𝓐𝓛𝓘𝓔𝓝 𝓐𝓛𝓕𝓐', '𝔸𝕃𝕀𝔼ℕ 𝔸𝕃𝔽𝔸', 'ⒶⓁⒾⒺⓃ ⒶⓁⒻⒶ', 'ALIEN ALFA', 'ᏗᏝᎥᏋᏁ ᏗᏝᎦᏗ', 'αℓเεɳ αℓƒα', '𝙰̷𝙻̷𝙸̷𝙴̷𝙽̷ 𝙰̷𝙻̷𝙵̷𝙰̷', 'ΔŁƗ€Ň ΔŁ₣Δ ', '𝐀𝐋𝐈𝐄𝐍 𝐀𝐋𝐅𝐀', '𝕬𝕷𝕴𝕰𝕹 𝕬𝕷𝕱𝕬', '🄰🄻🄸🄴🄽 🄰🄻🄵🄰', 'АГЇЄЙ АГFА', 'ᴬᴸᴵᴱᴺ ᴬᴸᶠᴬ'])
        ...
        try {
          ...
          //      console.log(m)
            ...
            let isROwner = [global.msgsz.user.jid, ...global.owner, ...global.devNum].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            let isOwner = isROwner || m.fromMe
            let isMods = isOwner || global.mods.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            let isPrems = isROwner || global.prems.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            let groupMetadata = (m.isGroup ? ((msgsz.chats[m.chat] || {}).metadata || await this.groupMetadata(m.chat).catch(_ => null)) : {}) || {}
            ...
            for (let name in global.plugins) {
                ...
                let _prefix = plugin.customPrefix ? plugin.customPrefix : msgsz.prefix ? msgsz.prefix : global.prefix
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (xp > 200) conn.reply(m.chat, 'Bonk! >.<') // Hehehe
                    else m.exp += xp
                    ...
                    try {
                        ...
                        if (e) {
                            ...
                            let teks = await msgsz.trans(lang, text).catch(async _ => await msgsz.trans2(lang, text))
            ...
            if (opts['typing']) msgsz.sendPresenceUpdate('composing', m.chat) 
    ...
    async participantsUpdate({ id, participants, action }) {
        if (opts['self']) return
        // if (id in msgsz.chats) return // First login will spam
        ...
        switch (action) {
            ...
            case 'remove':
                if (chat.welcome) {
                    let groupMetadata = await this.groupMetadata(id) || (msgsz.chats[id] || {}).metadata
                    for (let user of participants) {
                        let pp = 'https://avatars.githubusercontent.com/u/64305844?v=4'
                        let gif = await fetch("https://telegra.ph/file/ae73cc3c9128ad096104b.mp4")
                        try {
                            pp = await this.profilePictureUrl(user, 'image')
                        } catch (e) {
 
                        } finally {
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || msgsz.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', description) :
                                (chat.sBye || this.bye || msgsz.bye || 'Bye, @user!')).replace('@user', msgsz.getName(user))
                                img = profileimage
                                this.sendTBI2(id, text, wm,img,"Source Code",git,`Menu`, `.menu`,action === 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'ɢᴏᴏᴅʙʏᴇ', wm, {                      
                                    ...
                                    {
                                     jpegThumbnail: await (await fetch("https://telegra.ph/file/5351868fd5910de62e839.jpg")).buffer(), fileName: action === 'add' ? `Welcome ${this.getName(user)} 🥰` : `Goodbye ${this.getName(user)} ☺`, mimetype: 'image/jpeg', fileLength: global.fsdx, pageCount: global.pcdx,
                                     mentions: [user],
                                     contextInfo: {
                                     externalAdReply :{
                                        mediaUrl: insta,
               ...
               case 'credits':{
                    ...
                    break
               case 'promote':
                text = (chat.sPromote || this.spromote || msgsz.spromote || '@user ```is now Admin```')
            case 'demote':
                if (!text) text = (chat.sDemote || this.sdemote || msgsz.sdemote || '@user ```is no longer Admin```')
    ...
    async delete({ remoteJid, fromMe, id, participant }) {
        if (fromMe) return
        let chats = Object.entries(msgsz.chats).find(([user, data]) => data.messages && data.messages[id])
        if (!chats) return
...
global.dfail = async (type, m, msgsz) => {
    ...
    let msg = {
        ...
        private: `This Command Can Only Be Used In Private Chats @${msgsz.user.jid.split('@')[0]}`,
        admin: `This Command Only For Group Admin`,
        botAdmin: `This Command Activates When Bot Becomes Admin`,
        unreg: `Not Registered, Please Register By Typing #register name.age`,
    ...
    if (msg) return msgsz.reply(m.chat,  msg, m, { mentions: msgsz.parseMention(msg), jpegThumbnail: await (await fetch(pp)).buffer() })
}
...
global.tmbnld = [
    'https://github.com/Alien-alfa/Alien-alfa/blob/beta/Alfa.jpg?raw=true',
]
...
global.flaaa = [
    ...
    'https://flamingtext.com/net-fu/proxy_form.cgi?&imageoutput=true&script=spider-men-logo&doScale=true&scaleWidth=800&scaleHeight=500&text=',
    'https://flamingtext.com/net-fu/proxy_form.cgi?&imageoutput=true&script=clan-logo&doScale=true&scaleWidth=800&scaleHeight=500&fontname=ethnocentric&text=',
