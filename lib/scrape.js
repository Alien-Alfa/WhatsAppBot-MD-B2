    ...
    return new Promise((resolve, reject) => {
        fetch('https://jagokata.com/kata-bijak/kata-' + input.replace(/\s/g, '_') + '.html?page=1')
            ...
            .then(res => {
                ...
                data.splice(2, 1)
                if (data.length == 0) return resolve({ creator: '@Alien-Alfa', status: false })
                resolve({ creator: '@Alien-Alfa', status: true, data })
            }).catch(reject)
    })
}
...
function facebook2(urls) {
    return new Promise(async (resolve, reject) => {
       ...
       resolve({ status: getPost.status, creator: '@Alien-Alfa', hasil: hasil })
    })
}
