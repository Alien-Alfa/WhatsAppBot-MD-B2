...
module.exports = async function (m, msgsz = { user: {} }) {
  let _name = await msgsz.getName(m.sender)
  let sender = PhoneNumber('+' + m.sender.replace('@s.whatsapp.net', '')).getNumber('international') + (_name ? ' ~' + _name : '')
  let chat = await msgsz.getName(m.chat)
  // let ansi = '\x1b['
  let img
  try {
    if (global.opts['img'])
  ...
  let me = PhoneNumber('+' + (msgsz.user && msgsz.user.jid).replace('@s.whatsapp.net', '')).getNumber('international')
  console.log(`
...
`.trim(),
  ...
  if (typeof m.text === 'string' && m.text) {
    ...
    if (m.mentionedJid) for (let user of m.mentionedJid) log = log.replace('@' + user.split`@`[0], chalk.blueBright('@' + await msgsz.getName(user)))
    console.log(m.error != null ? chalk.red(log) : m.isCommand ? chalk.yellow(log) : log)
  }
  if (m.messageStubParameters) console.log(m.messageStubParameters.map(jid => {
    jid = msgsz.decodeJid(jid)
    let name = msgsz.getName(jid)
